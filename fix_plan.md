...
- [ ] P1 — Add `"capybara"` to `CURATED_SUBJECTS` in `volly/loop.py` and to the curated list in `specs/00-overview.md`. Also add `"owl"` and `"mushroom"` — silhouettes that reward shaded ASCII. Update `volly/ui/app.py` selectbox source if it's not already pulling from `CURATED_SUBJECTS` — spec: specs/00-overview.md
- [ ] P1 — Add `DEMO_PROMPTS["capybara"]` (and `"owl"`, `"mushroom"`) entries — each ~10 lines, mentions the subject ≤2×, suggests a shaded character palette `(. , : ; - = + * # @)` for tonal range. These are the "rehearsed" prompts the operator uses with `--demo` for predictable on-stage output — spec: specs/00-overview.md §"Demo mode"
