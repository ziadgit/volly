...
tests pass, ruff clean, import OK). The only remaining unchecked item
...
**Discovered** spec-wording-drift items landed 2026-05-23: judge fallback
rewritten to "uniform fallback" with cross-reference to the ablation
hook; free-tier RPM narrative reconciled across specs 00/02/03 to
explain the `--tier free` preset's rpm=4 (one below the 5 RPM ceiling
for safety margin); spec 03 `GeminiClient` public surface updated to
include the `temperature` kwarg on `multimodal()` and `json()` (already
...
- [x] P2 — **Spec wording drift on judge fallback.** `specs/02-loop.md` §"Failure handling" line 111 says "Judge returns malformed JSON twice in a row → fall back to **text-only judge** for that iteration only", and `specs/06-judge.md` line 46 says "fall back to **text-judge mode** (`prompt_suggestions=[]`, `scores=uniform`)" — but the actual `volly/judge.py:_fallback_result` returns uniform 0.5 scores WITHOUT invoking text-judge. The phrase "text-judge mode" now collides with the `--ablate-judge` text-judge feature (spec 06 §"Ablation hook"), making the failure-handling intent ambiguous: is the fallback supposed to call `rank(..., include_images=False, texts=[...])` (real text-judge, could itself 429) or just synthesize uniform scores (current, safer)? Code's choice (uniform synthesize) is the right one for demo robustness — a text-judge fallback that itself fails would cascade. Fix the SPEC, not the code: rewrite spec 02 line 111 to "fall back to **uniform 0.5 scores** for that iteration only" and spec 06 line 46 to "fall back to **uniform fallback** (...)" — keep the parenthetical, drop the misleading "text-judge mode" phrase. Cross-reference the ablation hook so future readers don't conflate the two paths — spec: specs/02-loop.md, specs/06-judge.md
- [x] P2 — **Spec wording drift on free-tier RPM.** `specs/00-overview.md` (lines 73-74, 82) and `specs/03-gemini-client.md` line 96 prescribe `--rpm 5` for free-tier operators, and `specs/02-loop.md` line 67 says "set 5 for free tier" — but the `--tier free` preset table at `specs/02-loop.md` line 127 sets `--rpm 4`, and `volly/loop.py:190` (`_TIER_PRESETS["free"]`) implements that 4. The preset value is correct as a safety margin (one below the 5 RPM Gemini ceiling, defending against burst/clock drift), but the spec never explains the 4-vs-5 choice, so a future loop could "fix" the preset to 5 or misread the narrative. Resolve in spec: update spec 02's `--rpm` flag doc + add an explanation paragraph after the preset table; update specs 00 and 03 to recommend `--tier free` (rpm=4) as the canonical free-tier path; update `volly/loop.py:716` argparse help text to match. Operator override `--tier free --rpm 5` remains supported. Code unchanged (preset value is the truth) — spec: specs/00-overview.md, specs/02-loop.md, specs/03-gemini-client.md
- [x] P2 — **Spec-vs-code drift on `GeminiClient` public surface.** `specs/03-gemini-client.md` §"Public surface" shows `temperature: float = 1.0` on `text()` (line 32) but omits it from `multimodal()` (line 35-42) and `json()` (line 44-53) — yet `volly/gemini_client.py:338,352` implements `temperature: float = 1.0` on both, threaded through `_build_config` exactly like `text()`. No caller in the package passes `temperature=` explicitly, so this is doc-only drift (no behavioral change), but the spec is the contract a future loop reads when adding a caller that wants temperature control (e.g. a deterministic-ablation flag). Fix in SPEC, not code: add the `temperature: float = 1.0` kwarg to both method signatures. Code unchanged — spec: specs/03-gemini-client.md
